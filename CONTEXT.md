...
`script/test`, `script/release`, `script/watch` (real directories, no
`dist/` origin).
...
(`migrated_files`) and the resync stages exactly that, so the heal reaches
the caller's commit whichever release is driving (#1036).
