...
# why: The base case the rest deviate from: the lifted script is the step's
# own body, so a spec can RUN what the workflow runs instead of asserting
# against a copy of it that drifts.
...
# why: A step is only obliged to carry a `name:`. Requiring an `id:` would
# mean editing the workflow to make it testable, and the step under test
# would no longer be the step that runs.
...
# why: The load-bearing case for every caller's `[ -s ... ]` guard. yq
# answers a missing key with the literal `null` at status 0, which is a
# one-line script the guard accepts and bash runs as a command -- so a
# spec asserting "this step fails" passes on `null: command not found`
# against a workflow whose step runs no shell at all.
...
# why: The other end of the same contract -- a step name that resolves to
# nothing (a rename, a typo) must be empty output, which is what turns a
# caller's guard into a loud failure instead of a silently empty run.
