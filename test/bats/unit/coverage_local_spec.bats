...
# single-threaded, and base#1060 measured what that costs: `bats --jobs`
# under ONE kcov reproduces a SHARD's covered set but not the WHOLE
# SUITE's, so it is on for shards and off for the full run. The bound is
# trace VOLUME through the one parser, which leaves the full run exactly
# one parallelism: N independent kcov PROCESSES over disjoint slices,
# merged -- one parser per slice instead of one for all of them.
#
