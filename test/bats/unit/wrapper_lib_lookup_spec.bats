...
#
# Pre-fix the wrappers hard-coded ${FILE_PATH}/template/dist/script/docker/lib/_lib.sh,
...
setup() {
  ...
  for _w in build.sh run.sh exec.sh stop.sh; do
    ln -s "/source/dist/script/docker/wrapper/${_w}" "${SANDBOX}/${_w}"
  done
}
...
@test "build.sh: errors clearly when neither .base/ nor sibling _lib.sh exists (#282)" {
  ...
  assert_output --partial ".base/dist/script/docker/lib/_lib.sh"
}
