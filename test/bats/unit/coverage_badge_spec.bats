...
@test "coverage_badge: the generator header records the bump wiring as pending" {
  ...
  # ... and must not state the wiring as an accomplished property. The
  # rejected phrasing named the caller with no qualification at all; a
  # qualified "is TO BE regenerated by the release bump ... tracked in
  # docker_harness#289" is the correction, not a repeat.
