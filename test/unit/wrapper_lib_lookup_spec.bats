...
#     _lib.sh from .base/downstream/script/docker/lib/_lib.sh and print usage. The lookup
...
setup() {
  ...
  for _w in build.sh run.sh exec.sh stop.sh; do
    ln -s "/source/downstream/script/docker/wrapper/${_w}" "${SANDBOX}/${_w}"
  done
}
...
@test "build.sh: errors clearly when neither .base/ nor sibling _lib.sh exists (#282)" {
  rm -f "${SANDBOX}/.base/downstream/script/docker/lib/_lib.sh"
  run "${SANDBOX}/build.sh" --help
  assert_failure
  assert_output --partial "cannot find _lib.sh"
  assert_output --partial ".base/downstream/script/docker/lib/_lib.sh"
}
