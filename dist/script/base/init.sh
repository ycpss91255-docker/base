...
_create_new_repo() {
  ...
  # setup.toml; _call_setup at the end of this flow will emit it.
...
#       touches at all;
#     - setup.toml, which setup.sh writes on a first-time bootstrap or a
#       stale-mount_1 rewrite and leaves alone otherwise.
#
#   The ignore pair and setup.toml are why this list is CONDITIONAL and
...
#   that adds a canonical entry, and setup.toml is rewritten in place by a
...
#     the setup.toml relocation    setup.toml, the name it moves the
...
_init_existing_repo() {
  ...
  # And, for the same reason one line up, the per-repo `setup.toml`
  # override that moved out of `config/` to the repo root. This is
...
#   workflow, .hadolint.yaml, the two ignore files and setup.toml, each of
...
#   _call_setup records `setup.toml` across a separate process.
#
...
_sync_existing_gitignore() {
  ...
  # current setup.toml. Used to live in setup.sh apply (runtime); now
  # tied to init/upgrade lifecycle so the file stays consistent even
  # when setup.toml changed between wrapper invocations.
  _sync_logging_gitignore "${REPO_ROOT}"
...
# ── Generate per-repo setup.toml ────────────────────────────────────────────
#
# Copies <subtree-prefix>/setup.toml to <repo>/setup.toml
...
_gen_setup_conf() {
  local _src="${TEMPLATE_DIR}/dist/setup.toml"
  local _dst="${REPO_ROOT}/setup.toml"
  local _force="${1:-false}"
  # setup.toml is a repo-root file — no parent dir to create.
  if [[ ! -f "${_src}" ]]; then
    _error "Template setup.toml not found at ${_src}"
  fi
  if [[ -f "${_dst}" ]]; then
    if [[ "${_force}" != "true" ]]; then
      _error "setup.toml already exists at ${_dst}. Remove it first or edit directly."
    fi
    ...
    local _bak="${_dst}.bak"
    cp -f "${_dst}" "${_bak}"
    _log "Backed up existing setup.toml → ${_bak}"
...
# _call_setup
#   Run setup.sh over this repo, and record whether it wrote `setup.toml`.
#
#   `setup.toml` is a published installed path, so the staging step has to
...
_call_setup() {
  local _conf="${REPO_ROOT}/setup.toml"
  ...
  [[ "${_after}" == "${_before}" ]] || _init_record_write "setup.toml"
  return 0
}
...
Options:
  --gen-conf         Copy <subtree-prefix>/setup.toml to
                     <repo>/setup.toml so the user can
                     override any section (image_name / gpu / gui /
                     network / volumes / security / stage:*). Refuses
                     to overwrite an existing per-repo setup.toml unless
                     --force is given.
  --force            With --gen-conf: overwrite existing setup.toml,
                     backing up the previous setup.toml to setup.toml.bak
...
EOF
  ...
  # `setup.toml` is written there and it is one of the paths
