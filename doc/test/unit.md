...
| `doc/test: no catalogue points at a grand total TEST.md no longer records (#978)` | The figure and the POINTERS to it are one shape, and a branch that removes the first without the second leaves the documentation worse than it found it: a live cross-reference to a number nobody can find reads as the reader's failure to look. The premise is asserted rather than assumed, from the tree: this case only forbids the pointers because TEST.md itself records no total, so restoring the figure lifts the rule instead of leaving a rule nobody can satisfy. |
