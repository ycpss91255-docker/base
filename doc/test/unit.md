...
Unit specs under `test/bats/unit/`: **3780 tests**.
...
cases pin what it answers -- including that it REFUSES a ref it cannot read
as a version tag, because the alternative answer (`false`) is the branch
that publishes. The last three derive the population of asking sites from
`.github/workflows/` rather than listing it: a guard for "every site" that
consulted a remembered list of three would pass on exactly the fourth.
...
| `release-ref: every workflow that declares a prerelease: input calls the classifier (#1012)` | The load-bearing half of "one rule, one home": the asking population and the calling population are the same set, both derived from the tree. |
...
| `test-tools pins: check accepts the exact declared version (#1012)` | The ordinary agreement, without which every refusal below could be a rule that refuses everything. |
| `test-tools pins: check accepts a longer version under a series pin (#1012)` | `ALPINE_VERSION=3.21` against `/etc/alpine-release`'s `3.21.7`: an equality rule would fail every image this Dockerfile can build. |
