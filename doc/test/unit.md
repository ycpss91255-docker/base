...
### test/bats/unit/ci_spec.bats (151)
...
| `_bats_args_with_label: --jobs is a floor under the core count, never a multiple of it (#1068)` | the floor has to be a FLOOR. `k x nproc` reproduces the right answer on a 4-core runner by coincidence and prescribes 128 on a 32-core host, where 128 measured worse or equal to 32 (60.7 / 47.3 against 49.0 / 47.3) -- so a large machine gets exactly its cores, and the rule stays monotone so a bigger host is never handed less than a smaller one. This is the assertion that lets the constant be re-tuned without editing a test: never-below-the-machine, non-decreasing, and equal at the top name nothing the measurement picked. |
