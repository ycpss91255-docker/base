...
| `errexit-bang` | no `!` statement outside the LAST statement of any `*.bats` body in the repo, and none handing its verdict on via a `;`, an async `&` or an `\|\| true` anywhere in it (continuation lines included, up to the blank or comment line that ends the statement). The judgement is made on the FOLDED statement -- physical lines are joined while a `\` continuation, a quote or a `(` is still open, and the scan then runs once from the first character -- so a separator inside a `( ... )` is the argument's wherever the `(` and its `)` sit. A statement whose quote or `(` never closes before its body does is REPORTED when it is a `!` one or when it folded a line opening with `!` into itself; otherwise it is unreadable but provably hid nothing this rule judges, which is stated in the driver rather than claimed away; the population is derived by walking the tree, not listed: `test/bats/` and the shipped `dist/test/bats/smoke/` both count | `lint-static (errexit-bang)` | ungated |
