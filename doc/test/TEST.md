...
Template self-tests: **4171 tests** total (4006 unit + 165 integration).

> "Self-test total" is the `just test` suite -- what runs in the
> `Self Test` CI job. System (19) and smoke (39) tests are tracked here
> too but are **not** in the 4171 figure: System specs need host docker
...
| Doc | Scope | Count |
|-----|-------|-------|
| [unit.md](unit.md) | `test/bats/unit/` -- library, wrappers, generators, templates (Unit level) | 4006 |
| [integration.md](integration.md) | `test/bats/integration/` -- init / upgrade / dispatch across components (Integration level) | 165 |
...
Self-test grand total (unit + integration): **4171**.
...
| `changelog-entry` | no `[Unreleased]` changelog entry over 700 chars (measured whitespace-collapsed over the whole entry), no entry repeating another's lead bullet, no `### <category>` heading opening twice in one release block, and no category heading outside the locked roster in `script/release/changelog_categories.sh` -- which `doc/changelog/CONVENTIONS.md` must print unchanged | `lint-static (changelog-entry)` | ungated |
