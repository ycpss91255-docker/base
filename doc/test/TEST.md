...
Template self-tests: **2524 tests** total (2406 unit + 118 integration).

> "Self-test total" is the `just test` suite -- what runs in the
> `Self Test` CI job. System (9) and smoke (40) tests are tracked here
> too but are **not** in the 2524 figure: System specs need host docker
...
| Doc | Scope | Count |
|-----|-------|-------|
| [unit.md](unit.md) | `test/bats/unit/` -- library, wrappers, generators, templates (Unit level) | 2406 |
...
Self-test grand total (unit + integration): **2524**.
...
before it existed, and `home-literal` joined the matrix in the same change
that introduced it.
