...
### Changed
- **finish the setup.sh-derived spec re-split: one spec per leaf lib, retire the god-files (P1b, closes #758, ADR-00000015)** — final, behaviour-identical relocation; every moved `@test` is byte-identical (same name / body / assertions) and the unit grand total is unchanged. Per ADR-00000015 (test files mirror source; isolated unit tests move to the leaf lib's spec; integration tests stay with the orchestrator), the two remaining god-files `setup_spec.bats` (147) and `setup_emit_spec.bats` (72) are split into new flat specs mirroring `lib/`: `resolve_spec.bats` (25 — `_resolve_*` / `_detect_jetson` / `_detect_dri_groups` incl. the #760 `SETUP_DETECT_*` operator-override contract / `_compute_conf_hash`), `setup_detect_spec.bats` (45 — `detect_*` / `_is_ssh_x11` / `detect_image_name` / `detect_ws_path` / `_reconcile_workspace_path`), `setup_conf_spec.bats` (12 — `_load_setup_conf` / `_get_conf_*` / `_rule_basename`), `env_emit_spec.bats` (4 — `write_env` / `_scaffold_env_overlay`), and `drift_spec.bats` (4 — `_check_setup_drift`); the residual `_yaml_dq` test (compose_emit-owned) moves into the existing `compose_emit/blocks_spec.bats` (27 -> 28). `setup_emit_spec.bats` is removed (its `apply`-pipeline / CLI-flag / i18n tests fold back into the orchestrator `setup_spec.bats`, its leaf tests move out). `setup_spec.bats` is slimmed to the orchestrator (147 -> 128): `main` dispatch, `usage`, `_setup_msg` / `_msg`, the `_parse_ini_section` / `_ini_tokenize` / `_setup_known_section` / `_setup_ssh_x11_cookie` units not owned by a P1b leaf, and the `apply` pipeline integration tests (driving detect -> resolve -> write_env -> compose emit end-to-end). Count math: 147 + 72 = 219 redistributed as 128 + 25 + 45 + 12 + 4 + 4 + 1 = 219; the unit-dir total is net-zero. `unit.md` resynced. Closes #758.
