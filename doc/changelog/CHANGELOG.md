...
- **`just template new <name>` scaffolds repo-local command groups (#633, epic #625, ADR-00000010, closes #594)** — Phase-2. Adds the `template` namespace (`mod? template` in the entry): `just template new <name>` creates `script/local/<name>/justfile.<name>` + `<name>.sh` from `downstream/script/template/skel/` (substituting `__NAME__`) and appends the group's `mod?` line to `script/local/justfile.local`, so the group is immediately usable as `just <name> <recipe>`; bare `just template` prints help. The scaffolder `new.sh` validates the name, refuses to clobber an existing group, and keeps the registry append idempotent. The `new` recipe runs `cd {{justfile_directory()}} && script/template/new.sh ...` -- `justfile_directory()` is the repo root in both a bare invocation and under `--working-directory` (unlike `[no-cd]` / `invocation_directory()`), so the scaffolder always runs from a known-good cwd. `init.sh` symlinks `script/template/{justfile.template,new.sh,skel}` (base-owned, flow on upgrade) and `ci.sh` shellchecks `downstream/script/template/`. This closes #594's "extension point for repo-local recipes" -- superseding its original `import?`-a-justfile plan, which `just`'s duplicate-recipe-name hard error makes unworkable (namespacing via `mod?` is required). Tested: `template_new_spec` (7 unit, run `new.sh` directly), `justfile_user_spec` (executable: `just template new` -> group works, bare help), `init_new_repo` (template symlinks), `TEST.md`. README docs of the feature land with the #644 holistic refresh (4-lang lockstep). Refs #625, ADR-00000010. Closes #594.
...
- **layered consumer just entry: split into `downstream/script/justfile` (entry) + `downstream/script/docker/justfile.docker` (#631, epic #625, ADR-00000010)** — Phase-2 tracer bullet. The single downstream `justfile` becomes a thin **entry** that `import`s a **docker module**: the verb recipes (`build`/`run`/`exec`/`stop`/`prune`/`setup`/`setup-tui`/`upgrade`/`upgrade-check`) move to `downstream/script/docker/justfile.docker` (imported TOP-LEVEL, so `just build` still works at the repo root), and the `default` recipe lives in the entry (the docker module carries no `default` -- `just` treats a duplicate recipe name across an import as a hard error). `init.sh` now wires the symlink chain `<repo>/justfile -> script/justfile -> .base/downstream/script/justfile` plus `<repo>/script/docker/justfile.docker -> .base/downstream/script/docker/justfile.docker` (the entry's `import 'script/docker/justfile.docker'` resolves relative to the repo root, the symlink location -- verified). This is the base for the namespaced extensions (#632 `local`, #633 `template`, #634 `ci`/`cd`). `justfile_spec` (static: entry vs module) + `justfile_user_spec` (executable: full chain fixture) + `init_spec`/`init_new_repo`/`gitignore_sync`/`upgrade_spec` fixtures + `TEST.md` updated; also folds in the P1.4 `TEST.md` smoke-path debt (`test/smoke/*` headers -> `downstream/test/smoke/*`). **Deferred (still):** README ASCII tree + mermaid diagrams -- the `justfile -> .base/script/docker/justfile` mermaid edge is now stale and is part of the #644 holistic refresh. Refs #625, ADR-00000010.
- **base self-CI is now the `just ci` namespace; `justfile.ci` → `script/ci/justfile.ci` + base-own root `justfile` entry (#630, epic #625, ADR-00000010)** — fifth slice of the base/downstream split (base-own side). The root `justfile.ci` (invoked `just -f justfile.ci <recipe>`) moves to `script/ci/justfile.ci` and is reached via a new base-own root `justfile` entry that `mod?`s it as the **`ci`** namespace, so base maintainers run `just ci test` / `just ci lint` / `just ci coverage` / `just ci upgrade` (the `-f`-a-separate-file pattern was a Makefile-era holdover; `just` namespaces it). A `script/cd/justfile.cd` skeleton is added as the `cd` namespace (CI and CD kept separate; real release/publish recipes deferred). `set working-directory := '../..'` in both module files pins recipes to the repo root whether reached via the module or `just -f`. The base-own root `justfile` is distinct from the shipped consumer entry (`downstream/script/justfile`, #631) and lands harmlessly at `.base/justfile` in a consumer. README (×4) + `CONTEXT.md` + `self-test.yaml` (comment + release-archive: `justfile.ci` → root `justfile`, `script/` already carries `script/ci/`) + `template_spec`/behavioural specs rewired to `just ci` / `script/ci/justfile.ci`. **Deferred:** the README ASCII structure-tree + mermaid diagrams (stale across the whole P1.x reorg) get a single holistic refresh after the downstream/ tree settles (post-#634); the **harness** `CLAUDE.md` `just -f justfile.ci` references are a separate-repo doc update (pending owner consent). Refs #625, ADR-00000010.
- **shipped smoke templates + `.hadolint.yaml` relocated to `downstream/` (#629, epic #625, ADR-00000010)** — fourth slice of the base/downstream split: base's shipped `test/smoke/` (the shared `test_helper.bash` + `display_env.bats` / `script_help.bats` templates) moves to `downstream/test/smoke/`, and `.hadolint.yaml` to `downstream/.hadolint.yaml`. base-own tests (`test/{unit,integration,behavioural}`) stay. Rewires `init.sh`'s `.hadolint.yaml` symlink **source** (the consumer's `<repo>/.hadolint.yaml` symlink + `<repo>/test/smoke/` overlay are unchanged), the consumer `Dockerfile`'s `COPY .base/test/smoke/` layer (the sibling `COPY test/smoke/` for the consumer's own smoke stays), and the base-own spec references (`smoke_helper_spec` load path, `template_spec` existence checks, `init_spec`'s `.hadolint` symlink-target fixture + assertions). `release-worker.yaml` is consumer-context (repo-root `test/smoke/` + `.hadolint.yaml`) so its archive list is unchanged. The base-own `self-test.yaml` is rewired: the dedicated `hadolint` job's `config:` → `downstream/.hadolint.yaml` (CI-only, not in the local bats gate) and the `release` job's template-archive `cp -r` list now includes `downstream/` and drops the relocated `.hadolint.yaml`/`config/` (the latter a P1.1 follow-up the local gate could not catch). No downstream-visible behaviour change until the fanout (#636). Refs #625, ADR-00000010.
- **consumer Dockerfile template relocated + renamed to `downstream/dockerfile/Dockerfile` (#628, epic #625, ADR-00000010)** — third slice of the base/downstream split: `dockerfile/Dockerfile.example` moves to `downstream/dockerfile/Dockerfile` (the `.example` suffix dropped — the `downstream/` location now disambiguates it from the base-own `dockerfile/Dockerfile.test-tools`, which stays). Rewires `init.sh`'s new-repo `cp` source, the `self-test.yaml` hadolint/integration paths, and all spec/fixture path references (`init_spec`'s stub now `mkdir -p`s the `downstream/dockerfile` parent). README (×4) + `CONTEXT.md` synced. Test titles/prose that name "the example Dockerfile" are left as-is (descriptive, not paths). No downstream-visible behaviour change until the fanout (#636). Refs #625, ADR-00000010.
- **base docker library/runtime/wrappers relocated to `downstream/script/docker/` (#627, epic #625, ADR-00000010)** — second slice of the base/downstream split: `script/docker/{lib,runtime,wrapper}` move to `downstream/script/docker/{lib,runtime,wrapper}` (the user-facing `justfile` stays at `script/docker/justfile` until #631). Rewires every consumer-side and base-own reference: `init.sh` wrapper-symlink sources + lib source + entrypoint copy; `upgrade.sh` lib source + **Region C** Dockerfile lint-stage auto-patch (`COPY .base/script/docker/lib` → `.base/downstream/script/docker/lib`) in lockstep per ADR-00000006; `ci.sh` + `lint_*.sh` lib source + shellcheck scan roots; `bootstrap.sh` consumer `_lib.sh` lookup + the wrappers' bootstrap-locator loop + `build.sh`/`run.sh` sibling `setup.sh` invocation; `setup.sh` default-`base_path` BASH_SOURCE walk (now one level deeper) + its template-conf locator; `Dockerfile.example` lint-stage `COPY`s; the `self-test.yaml` classify block-list; and all `# shellcheck source=` directives. `_lib.sh`'s sub-lib sourcing is `BASH_SOURCE`-relative so the lib tree is self-consistent after the move. README (×4) + `CONTEXT.md` wrapper glossary synced. Affected unit/integration specs + fixtures updated. No downstream-visible behaviour change until the fanout (#636). Refs #625, ADR-00000010, ADR-00000006.
- **base config default-layer relocated to `downstream/config/` (#626, epic #625, ADR-00000010)** — first slice of the base/downstream directory split: base's shipped config tree (the `Dockerfile` layer-1 default layer + the `--gen-conf` source + `setup.sh`'s template baseline) moves from `config/` to `downstream/config/`, so consumers read it from `.base/downstream/config/`. Rewires the consumer `Dockerfile.example` layer-1 COPY, `init.sh`'s `--gen-conf` source, `setup.sh`'s template-baseline locator (`../../../config` → `../../../downstream/config`), `ci.sh`'s base-own shellcheck/test paths, and — per ADR-00000006's frozen-`.base`-path discipline — `upgrade.sh` Region B config-drift detection (`${TEMPLATE_REL}/config` → `${TEMPLATE_REL}/downstream/config`) in lockstep, so the drift warning keeps firing. The consumer-side `<repo>/config/` overlay path is unchanged; there is no downstream-visible behaviour change until the fanout (#636). Affected unit/integration specs updated to the new shipped location. Refs #625, ADR-00000010, ADR-00000006.
