...
### Fixed
- **a failed diff no longer takes the required `docker-build` check green, and one FROM-line reader replaces three (closes #1013)** -- the doc-only classifier read its diff through `done < <(git diff ...)`, and a loop's status is the loop's: a force-pushed base or a shallow clone delivered zero paths, classified doc-only, and passed the required check having built nothing. It is captured and checked now. Separately, the extra-stages loop and `runtime_stages.sh` each carried a `FROM ... AS` regex whose comment claimed it matched the stage parser; neither did, and `FROM --platform=... AS x-test` was invisible to the loop. Both read one roster through `stage_names.sh` now.
