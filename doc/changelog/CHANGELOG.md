...
- **`Makefile.ci` retired for `justfile.ci`; make dropped as a second runner (#573, ADR-00000005 amendment)** — ADR-00000005 moved container-ops off make but kept the base self-test gate `Makefile.ci` on make (per #475), leaving the repo with two runners. This converges on a single runner: a base-local `justfile.ci` (invoked `just -f justfile.ci <recipe>` — not auto-discovered, so it never collides with the container-ops `justfile`) maps the former make targets 1:1 (`test` / `test-behavioural` / `lint` / `coverage` / `clean` / `init` / `upgrade` / `upgrade-check`), and `Makefile.ci` is deleted. The integration tests already exercised the downstream `just upgrade-check` (#546), so the `make` install in `ci.sh` (kcov path) and `Dockerfile.test-tools` was dead weight and is removed (the release-test-tools smoke no longer probes `make --version`). Stale container-ops hints (`setup.sh` / `setup_tui.sh` `make run`/`make exec`, `run.sh` auto-build comment, `gitignore.sh` `make upgrade`, `setup.conf`) and the `make -C` half of the `-C`-flag analogy in the wrappers are rewritten to `just`. Docs synced: ADR-00000005 carries a dated superseding amendment, README (4 locales) + TEST.md point at `just -f justfile.ci`. The `make -f Makefile.ci upgrade VERSION=vX.Y.Z` invocation becomes `just -f justfile.ci upgrade vX.Y.Z` (positional, no `VAR=VALUE`). Harness-side (`enforce_make_first_upgrade.sh`, CLAUDE.md / CONTEXT.md) and the downstream fanout are tracked separately. Closes #573.
