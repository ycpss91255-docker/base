...
- **CI checks the tool VERSIONS in a pulled `test-tools:main`, not just their
  presence (refs #947)** -- a PR that leaves the test-tools Dockerfile alone
  reuses the rolling `:main`, which is republished only by a push to main
  touching that file. Between a pin bump and that republish, an unrelated PR's
  lint jobs ran under the rule set the repo had just moved off -- green, and
  reporting on the wrong thing. The obtain step now compares the image's
  shellcheck / hadolint against the pins in the checkout and rebuilds from
  source when they disagree. The probe is one script
  (`script/ci/probe_test_tools.sh`) instead of the five copies that hid the
  gap.
