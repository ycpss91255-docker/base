...
### Added
- **`setup.sh reset` subcommand** (#49 Phase B-4). Overwrites `setup.conf` with the template default, archiving the prior `setup.conf` to `setup.conf.bak` and the prior `.env` to `.env.bak` for one-shot rollback. Mirrors what `build.sh --reset-conf` does today, but exposes it as a setup.sh subcommand for scripted use. Without `--yes`, prompts for confirmation; non-tty without `--yes` refuses to proceed (safety guard against pipeline mishaps). Does NOT regenerate `.env` — chain `setup.sh apply` afterwards (or `build.sh` / `run.sh` will trigger drift-regen on next invocation since the conf hash will have changed).
