...
### Fixed
- **field compose dropped the watchdog, the ENV bake and the configured restart policy (closes #840)** — three defects of the same shape in the field-deploy emitter: `_resolve_deploy_context` computed a value that `_generate_resolved_compose` / `_generate_deploy_bundle` never consumed, so the dev compose honoured it and the field bundle silently did not. (1) **Watchdog env is now emitted into the field service `environment:`**, reusing `_emit_watchdog_env` exactly as the dev path does; the `environment:` header (previously emitted only under GUI) is now emitted for whichever of GUI / watchdog is present, and stays absent when neither is (no empty block). This is the field-critical one: `restart:` only recovers a container that **exits**, while the watchdog is the only mechanism that recovers a service that is **alive but wedged** (`realsense_ros1#81` documents exactly that on real hardware). Delivered through compose rather than baked `ENV` so the thresholds stay visible and adjustable in the self-contained bundle. (2) **The `[environment]` ENV bake follows the stage being built.** `_generate_runtime_dockerfile` matched the literal `AS runtime`, but `runtime` is only the template's example name -- a downstream repo deploying its own stage name silently lost every `[environment]` value from the image (and `_generate_resolved_compose` sets `env_top=""`, so compose did not re-emit them either), while the sibling `_bake_config_copy` / `docker build --target` already followed `${_stage}`. The stage is now a required 4th argument, and the `FROM ... AS <stage>` token is captured and compared literally rather than spliced into the pattern. (3) **The conf's `[lifecycle] restart` is honoured on the deploy path.** The field default stays `unless-stopped` (auto-start on host reboot) for an operator who configured nothing, but an explicitly configured `no` / `on-failure:5` is now emitted verbatim instead of being silently overridden (`on-failure:N` YAML-quoted; a malformed hand-edited value falls back to the field default, mirroring `_emit_restart_line`'s apply-time guard). Telling "absent" from "explicitly `no`" required two changes: `_resolve_deploy_context` now reports `restart_policy` RAW (empty = key absent) with each consumer applying its own default (`setup_cmd.sh` supplies dev's `no`, so the dev compose is byte-identical), and the template `.setup.conf` ships `[lifecycle] restart` **commented out** instead of a literal `restart = no` -- the section-replace merge otherwise fed that template default in as an "explicit" value for every repo, collapsing the distinction before the resolver ever saw it. A downstream repo that carries a literal `restart = no` in its own `.setup.conf` now gets `restart: no` in its field bundle, as configured; delete the line (or set `unless-stopped`) to take the field auto-start default. Unit coverage in `deploy_spec.bats` (+7: the raw restart record, the watchdog record, the three compose emissions, the shared `environment:` header, the malformed-value fallback), `stage_spec.bats` (+3: bake into a caller-named stage, siblings untouched, absent stage no-op) and `setup_spec.bats` (the template-contract test), plus Integration coverage in `deploy_bundle_flow_spec.bats` (+2: watchdog + restart carried end to end into the bundle compose, and `[environment]` baked into a non-`runtime` stage).
