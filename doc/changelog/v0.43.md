...
- **the local `test-tools` tag now moves when a file the Dockerfile COPYs
  ...
  `dockerfile/Dockerfile.test-tools` alone, on a premise that stopped
  holding when the toml-bridge stage added a plain
  `COPY dockerfile/toml_bridge.py`. Editing that file left the tag put,
  the rebuild was skipped, and the suite reported on a bridge from before
  the edit -- a silent wrong answer. The digest now covers every context
  path the Dockerfile COPYs, derived from the COPY lines. An unmodelled
  flag, an unresolvable source, any unreadable input (the Dockerfile
  included), `ADD` and `ONBUILD` each refuse, naming it. A Dockerfile
  that COPYs no context keeps its tag.
