...
- **two `setup_tui.sh` functions nothing called (refs #1073)** --
  ...
  opened) and `_prompt_mount_with_picker` (a four-step mount picker with
  three specs and no production caller -- the mount editors all route
  through `_edit_list_entry`), plus the four `action.*` message keys the
  first one owned, in all four locales. A third candidate,
  `_edit_section_resources`, was checked and KEPT: no menu row reaches it,
  but `resources` is a `SCHEMA_SECTIONS` member, so `setup_tui.sh resources`
  dispatches to it through `main`'s direct-jump path. Nothing downstream
  calls any of these; a repo that carries `.base/` needs no action.
