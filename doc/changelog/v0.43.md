...
- **the repo-root `upgrade.sh` is back, so an upgrade no longer removes the
  ...
  README and usage text name. The guard is behavioural: the released-caller
  spec now upgrades TWICE, re-running the same command string against the
  tree the first run produced, so it names no path and covers the next
  frozen entry point for free. Affects any repo still sitting on a release
  that documents `./.base/upgrade.sh`.
