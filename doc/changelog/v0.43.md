...
- **a lint for CI jobs that run undeclared tools (refs #1080)** --
  ...
  read from the job's own shell and, for a host-direct
  `test.sh --<lint>-only` / `--lint-group` selector, from the source of the
  driver it names. Both populations are computed -- the tools from the pin
  roster, the jobs from the workflow directory -- so neither a new job nor a
  new pin can join in silence. The reader folds backslash continuations
  before it reads anything, so a wrapped selector cannot lose its demand and
  a wrapped argument cannot pose as a command; it accepts a trailing comment
  on a `jobs:` or job key rather than reading past it into the job above;
  and non-vacuity is asked of EVERY workflow file, so one file it could read
  no job out of is a refusal instead of a silent skip counted as scanned.
