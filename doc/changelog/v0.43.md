...
- **a lint refusing the next whole-tree scan in the coverage suite (refs #1075)**
  ...
  `*REPO_ROOT` at the live checkout. Nothing in it is a roster: the files come
  from `_COVERAGE_FULL_SUITE_POOLS`, the refused roots from `compose.yaml`'s
  bind for `.` plus the live `${REPO_ROOT}`, and it dies rather than reporting
  clean when a pool is missing, when no spec is found, or when no
  `*REPO_ROOT=` assignment exists anywhere -- the blind-detector case. It
  runs in a `lint-static` group like every other computed member.

### Changed

- **the coverage suite tests a lint against fixtures; the tree is the lint
  job's (closes #1075)** -- 23 specs ran a driver over the live checkout
  under kcov. One, `_run_errexit_bang: the real bats tree is clean`, was
  331s of a 501s coverage shard against a 211s median: 66% of the coverage
  matrix's critical path in one test, which no shard count splits. All 23 go;
  each duplicated a `lint-static` job. Measured, they covered 15 lines, since
  `script/test/` is outside the denominator; the 11 real ones -- four
  `changelog_index.sh` branches only the live changelog reaches -- are now
  asserted by fixtures in `changelog_index_spec.bats`. Local full-suite
  coverage 6m45s to 4m31s.
### Fixed
