...
- **base is a building block: both generate stages become parameterised APIs
  (ADR-00000036, refs #1087)** -- an orchestrator composing several instances of
  ONE repo hit four separate walls (#1051, #1056, #1062, #1064), all of them the
  same wall: ADR-00000022 carried per-instance variation in a `.env` overlay, and
  an interpolation can carry a value but not a shape. Build args, image rules and
  conditional blocks change which lines exist in the emitted file, so under an
  overlay-only contract they were permanently per-repo. The rule is now that
  anything base decides at build time or before the container starts must be
  reachable by the caller, and only what is internal to a running container is
  outside the contract. ADR-00000022 sections 1 and 3 and ADR-00000025 section 6
  are amended in place rather than superseded -- the overlay stays correct for
  value-shaped fields. No behaviour changes for an existing consumer: the
  default call reads `<repo>/.setup.conf.local` and writes to the repo root
  exactly as now.
