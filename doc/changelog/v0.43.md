...
- **the local `test-tools` tag now moves when a file the Dockerfile COPYs
  ...
  `dockerfile/Dockerfile.test-tools` alone, a premise that stopped holding
  when the toml-bridge stage added a plain
  `COPY dockerfile/toml_bridge.py`. Editing it left the tag put, skipped
  the rebuild, and the suite reported on a bridge from before the edit --
  a silent wrong answer. The digest now covers every context path the
  COPY lines name. An unmodelled flag, an unresolvable source, any
  unreadable input (the Dockerfile, and a directory the walk cannot
  enter, included), `ADD` and `ONBUILD` each refuse, naming it. A
  Dockerfile that COPYs no context keeps its tag.
