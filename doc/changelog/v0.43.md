...
- **the local `test-tools` tag now moves when a file the Dockerfile COPYs
  ...
  path the Dockerfile COPYs, derived from the COPY lines rather than
  listed. An unmodelled flag, an unresolvable source, an unreadable path,
  `ADD` and `ONBUILD` each refuse, naming the line. A Dockerfile that
  COPYs no context keeps its tag.
