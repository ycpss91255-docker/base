...
- **an upgrade no longer replaces the repo's own configuration with the
  template defaults (closes #1086)** -- the migration relocating a per-repo
  `setup.conf` out of `config/` shipped in `upgrade.sh`, which a
  cross-version upgrade never runs: the consumer's own vendored copy
  drives, and no release before `v0.42.0` carries it. A repo on `v0.41.0`
  upgraded with exit 0 and came out named after the directory it was
  cloned into, with an empty `[environment]`. It now runs from `init.sh`'s
  resync -- which every released driver re-runs, and which re-establishing
  a subtree also runs -- so recovery is automatic on the next `just base
  upgrade`. Two files present merge per section instead of warning. An
  override reached through a symlink relocates by CONTENT, because moving
  the pointer out of `config/docker/` re-aims it and leaves `.setup.conf`
  dangling; and the relocation writes to no index but the repo's own, so
  running it on a tree that sits inside somebody else's checkout no longer
  stages a rename in their repository.
