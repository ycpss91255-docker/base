...
- **the `[Unreleased]` category set is locked, and the split changelog has a
  lint (refs #926)** -- `script/release/changelog_categories.sh` is the one
  definition of the seven categories (BREAKING plus the Keep a Changelog six);
  the changelog-entry lint refuses any other heading in `[Unreleased]`, and
  fails if `doc/changelog/CONVENTIONS.md` stops printing the same list.
  Released sections keep the headings they shipped with. The new
  `changelog-layout` lint pins where a section, its compare link and
  `[Unreleased]` live, re-derives the index, and refuses a `## [` section in
  `CHANGELOG.md`: an index names the series, it does not hold them. Affects
  anyone writing a base changelog entry.
...
- **the changelog is one file per 0.Y series behind a generated index (refs
  ...
  which is where a new entry goes. The index block is generated -- refresh it
  with `just release changelog-index`, never by hand. Affects anyone writing a
  base changelog entry.
- **a release page carries the curated notes, and an extraction that finds
  nothing fails the release (refs #926)** -- `script/release/release_notes.sh`
  replaces the release job's inline `awk`, which wrote an empty file on no
  match and shipped a body of autogenerated PR titles with green CI. A final
  release's notes are the union of its RC sections, merged category by
  category, entries above the first category heading included, and
  `generate_release_notes` is off: the v0.42.0 body was 26,690 characters of
  which five lines were curated. It refuses a body over GitHub's
  125,000-character limit, and one carrying fewer entries than its sources.
