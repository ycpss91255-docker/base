...
- **type-aware TOML merge for config layers (refs #1130)** --
  `_conf_load_layers` now detects all-TOML layer chains and delegates
  merge to the containerised bridge instead of the INI section-replace
  path. The Python bridge gains `_merge_toml` with `--merge` CLI mode:
  tables (dict) get key-level merge (upper overrides only keys it
  defines), arrays of tables (list) get array replace (entire array
  from highest layer). `_emit_kv` extended to handle arrays of tables.
  Bash shim gains `toml_bridge_merge` mounting each file via `-v`.
  Eight unit tests with mocked docker cover scalar merge, array
  replace, mixed files, empty upper, missing sections, and the
  `_conf_load_layers` dispatch.
- **setup.toml as primary config source (refs #1132)** -- the conf
  layer chain now loads `setup.toml` / `setup.local.toml` instead of
  `.setup.conf` / `.setup.conf.local`. `_conf_load` already
  auto-dispatches `.toml` files to the containerised TOML parser, so
  the change is purely the file paths `_setup_conf_layers` constructs.
  `_is_self_managed_repo` and the wrapper bootstrap check also updated.
- **.env.toml + .env.local.toml wired (refs #1133)** --
  `_scaffold_env_local` creates `.env.local.toml` (TOML guidance) and
  `.gitignore` gains `setup.local.toml` + `.env.local.toml`. Legacy INI
  entries kept for transition.
- **compose/env/stage emitters read TOML (refs #1134, #1135)** --
  `compose_emit.sh`, `env_emit.sh`, `stage.sh`, and their wrappers now
  read `setup.toml` / `.env.toml` instead of `.setup.conf` / `.env`.
  Stale `.setup.conf` references in header comments and path
  construction updated across six lib files. Two test-header assertion
  fixes.
- **24-file .setup.conf reference cleanup (refs #1136)** -- every
  remaining `.setup.conf` filename reference in `init.sh`,
  `upgrade.sh`, `compose_emit.sh`, `conf.sh`, `conf_logging.sh`,
  `config_summary.sh`, `deploy.sh`, `dockerfile_migrate.sh`,
  `drift.sh`, and `_tui_conf.sh` replaced with the TOML equivalents.
  No behavioural change; pure rename of string literals and comments.
- **one-time INI-to-TOML migration converter (refs #1137)** --
  `ini_to_toml_migrate.sh` converts a `.setup.conf` to `setup.toml`
  in-place during `init.sh` resync. Handles `[section]` headers,
  `key=value` pairs (quoting bare values), `[[array]]` entries, and
  comment lines. Downstream repos running `just base upgrade` get
  their `.setup.conf` automatically converted on the next init cycle.
...
- **the PRD says how completeness is judged, without saying the number
  ...
  against what is used, 28 of 38. The rule is recorded, the figure is computed
  when read (invariant 10).

- **the PRD states what base is, and stops stating how (refs #1087)** -- Purpose
  now defines base by what it produces: a block usable alone and composable with
  others, where using one alone is N=1, not a special case. Invariants 3 and 6
  split into a fixed property and a swappable mechanism, the shape invariant 7
  already used. Subtree stays, recorded as a measured choice. Scope gains the
  splitting rule: more than one container is more than one block.

- **base is a building block: both generate stages become parameterised APIs
  (ADR-00000036, refs #1087)** -- an overlay carries a value but not a shape, so
  build args, image rules and conditional blocks could never differ between two
  instances of one repo. The rule is now that anything decided before the
  container starts must be reachable by the caller. ADR-00000022 s1/s3 and
  ADR-00000025 s6 are amended, not superseded; the overlay stays correct for
  value-shaped fields. No behaviour change for an existing consumer.

### Removed

- **two `setup_tui.sh` functions nothing called (refs #1073)** --
  `_item_action_menu`, an Edit / Remove / Back sub-menu no list editor ever
  opened, and `_prompt_mount_with_picker`, plus the four `action.*` message
  keys the first one owned, in all four locales. The picker is the one that
  matters to a reader: v0.40.0 announced it as the `[devices]` /
  `[volumes]` mount mode picker and closed #461 on it, but it has had no
  caller since the commit that added it -- the mount editors all route
  through `_edit_list_entry` -- so that feature was never reachable.
  `_edit_section_resources` was checked and KEPT: `setup_tui.sh resources`
  reaches it through `main`'s direct jump.

- **four more TUI functions nothing called, two of them orphaned by the
  removal above (refs #1073)** -- deleting `_prompt_mount_with_picker`
  took with it the only two call sites of `_tui_radiolist`
  (`lib/_tui_backend.sh`) and the only caller of `_assemble_mount_value`
  (`lib/_tui_conf.sh`), so both shipped with nothing left to call them.
  Widening the dead-code guard past `setup_tui.sh` to every shipped
  `_tui_*` file also found `_validate_enum` and `_mount_container_path`,
  dead in `lib/_tui_conf.sh` since long before this change. Their specs
  go with them; `_validate_mount`'s space-bearing-path refusal (#687),
  which only the assembler spec asserted, moves into `tui_spec.bats`.

### Fixed

- **the config writers appended INI into `setup.toml` (refs #1165)** --
  every `setup.sh set` / `add` / `remove` and the `mount_1` bootstrap
  in `setup_detect.sh` go through `_upsert_conf_value` /
  `_write_setup_conf`, which still wrote `mode = bridge` and bare
  `[volumes]` headers, so the next bridge parse refused the file
  setup.sh had just written. Both writers now emit TOML for a `.toml`
  destination: strings quoted (booleans and integers bare), numbered
  keys (`mount_N`, `arg_N`, `rule_N`, ...) routed to the `[[array of
  tables]]` the bridge numbers back, `["stage:x"]` / `"gui.mode"`
  quoted. Comments and layout are preserved; the frozen TUI's
  `.setup.conf` still comes out as INI.

- **an upgrade threw away the operator's `.env.local` (refs #1163)** --
  `init.sh`'s existing-repo resync called `_migrate_env_local_to_toml`,
  which moves `.env.local` aside to `.env.local.bak` and writes
  `.env.local.toml`. Nothing reads `.env.local.toml`: compose still
  loads `.env` plus `.env.local`, and the apply that follows the resync
  re-scaffolds `.env.local` as a comment-only file. Every downstream
  operator's env overrides therefore stopped applying at the next `just
  upgrade`, under a log line saying they "were converted". The call is
  removed; the converter stays in `ini_to_toml_migrate.sh`, uncalled,
  until the `.env.toml` / `.env.toml.local` readers land (refs #1168).

- **a failed TOML merge was reported as a configuration with nothing in
  it (refs #1162)** -- `_conf_load_layers` read the bridge through a
  process substitution and then returned success whatever happened, so a
  bridge exiting non-zero with an empty stdout produced an empty handle
  and every setting fell back to its default, silently. The status is now
  read and a failed merge is refused, naming the layers. The bridge
  itself had been left with two merge functions and two `main()` bodies
  by a line-level merge resolution -- one merge function and one `main()`
  now, with the array-of-tables numbering (`rule_1`, `arg_2`) and the
  lowercase `true` / `false` the shell side reads.

- **`toml_bridge.sh` could only dispatch to `docker run` (refs
  #1128)** -- the test-tools container has no docker socket, so the
  shim failed wherever the suite runs. `_toml_bridge_use_native()`
  now tries the installed `/usr/local/bin/toml-bridge` first and
  falls back to `docker run` only when that binary is absent or
  `TOML_BRIDGE_FORCE_DOCKER=1` is set. Alongside it: 135 duplicated
  lines removed from `dist/setup.toml`, the acceptance-test build
  context corrected (`dockerfile/` -> `.`), `python3` added to the
  test-tools apk install, and seven test files aligned to the TOML
  config format.
- **the local `test-tools` tag now moves when a file the Dockerfile COPYs
  changes (closes #1166)** -- the tag digested
  `dockerfile/Dockerfile.test-tools` alone, until the toml-bridge stage added
  `COPY dockerfile/toml_bridge.py`. An edit left the tag put, skipped the
  rebuild, and the suite tested the old bridge. The digest now covers every
  context path COPY names. An unmodelled flag, an unresolvable source, any
  unreadable input (the Dockerfile and a directory the walk cannot enter
  included), `ADD` and `ONBUILD` refuse, naming it, and `prune --tool-tags`
  stops on a live checkout it cannot resolve rather than retiring around it.
  A Dockerfile that COPYs no context keeps its tag.

- **`./setup_tui.sh project` died with a bash command-not-found (refs
  #1073)** -- the direct-jump gate accepted any `SCHEMA_SECTIONS` member,
  but `project` deliberately ships no TUI editor (its name belongs in the
  gitignored `.setup.conf.local`), so `main` jumped to a function that does
  not exist -- after the dialog probe and the seeding `setup.sh apply` run.
  The gate now reads the function table, so it cannot accept a name it
  cannot dispatch, and such a section is reported by name with exit 2
  instead of as an "unknown argument". Affects anyone who typed the
  section name the config file uses.

- **the dead-code guard could not see four shapes of dead function (refs
  #1073)** -- it counted mentions, so a function naming itself in its own
  `${1:?...}` message was its own caller (the idiom appears 81 times in
  `dist/`, and is how `_assemble_mount_value` went unnoticed), a pair of
  dead functions calling each other kept both alive, and every
  `"<name>_${` in the file became a blanket exemption -- including
  `rule_` and `_TUI_MSG_`, a config-key prefix and an array-name prefix
  that dispatch nothing. It now computes reachability from roots to a
  fixed point instead, with no exemption list, and refuses to report a
  clean tree when its own scan came up vacuous.
- **an upgrade no longer replaces the repo's own configuration with the
  template defaults (closes #1086)** -- the migration moving a per-repo
  `setup.conf` out of `config/` shipped in `upgrade.sh`, which a
  cross-version upgrade never runs: the driver is the consumer's own
  vendored copy, which predates it. A repo on `v0.41.0` upgraded with
  exit 0 and came out named after its own directory, `[environment]`
  empty. It runs from `init.sh`'s resync now -- which every driver and
  re-established subtree runs -- so recovery is automatic on the next
  `just base upgrade`. Two files present merge per section instead of
  warning, a symlinked override moves by content, and only this repo's
  index is written.
- **the `shellcheck` CI job runs the pinned binary (closes #1080)** -- it ran
  whatever `ubuntu-latest` shipped pre-installed, so the local gate, which
  runs the pinned v0.11.0 from the test-tools image, could not predict a
  required check in either direction: measured, 0.10.0 reports SC2120 on a
  shape v0.11.0 passes, and the runner image is rebuilt continuously. The job
  now installs `SHELLCHECK_VERSION` from the release tarball, read through
  `script/ci/test-tools-pins.sh` and never restated, and asserts the binary on
  PATH IS the pin before running the driver. No buildx and no image, so the
  job keeps its cost.

- **the repo-root `upgrade.sh` is back, so an upgrade no longer removes the
  command that performed it (closes #1077)** -- the `dist/` move left a
  forwarder at `init.sh` and none beside it, so a consumer on `v0.41.0` or
  earlier upgraded once and then got exit 127 from the command their own
  README and usage text name. Affects any repo still sitting on a release
  that documents `./.base/upgrade.sh`. A repo already stranded by the
  earlier upgrade has no `.base/upgrade.sh` to type: run `just base upgrade`
  once (or `./.base/dist/script/base/upgrade.sh [vX.Y.Z]` without `just`)
  and the forwarder comes back with it. The guard is behavioural: the
  released-caller spec now upgrades twice.


## [v0.43.0-rc2] - 2026-09-06
...
- **`lint-static` runs as four derived groups, not twenty one-lint jobs
  (closes #1071)** -- the 20 jobs each paid a runner startup to do a few
  seconds of work, and took concurrent slots from the coverage shards, the
  run's critical path. Four groups: no partition beats its longest driver,
  and four is the smallest count whose even share falls under that floor.
  The partition is round-robin and cost-blind, so the slowest group is that
  driver plus its neighbours -- above the floor, not at it. Membership is
  computed from `_LINT_TOOLS` by `test.sh --lint-group N/T`, so a new lint
  joins a group with no workflow edit. A checks-list entry names a group;
  its output names every lint that failed in it.
- **the bats job count is a floor on concurrency, not the core count
  (closes #1068)** -- `$(nproc)` answers a CPU-bound question and this suite
  waits on subprocesses: on CI's 4-core runner `--jobs 4` measured no faster
  than serial. Now `max(cores, 32)` -- a floor, not `k x nproc`: 32 is the
  knee (16 jobs cost 1.65x, 32/64/128 are within 3% of each other), and only
  a floor cannot land under it. 1.75x on a 4-core coverage shard, 2.1x on
  plain bats. The two kcov runners hold at one job per core under a new `metered`
  policy: more jobs there loses a reproducible 31 covered lines to kcov's one
  parser (#726). And a failed `nproc` no longer prints the label a genuine
  4-core reading prints.
...
- **a coverage run hands `coverage/` back whatever its verdict was, and
  `just test clean` can reclaim it when nothing did (closes #1032)** --
  `_fix_permissions` sat on the success path, so one red test in 4489 left
  the reports owned by root and the checkout unable to run coverage again:
  a host `rm` cannot unlink a file out of root's directory, so the clean
  failed too. The chown now runs from the EXIT handler, armed before the
  first phase, and never changes the run's status. `just test clean`
  removes the reports from inside a container over the same mount, against
  a hardcoded target, and fails loudly if anything is left. The refusal
  that blocks the next run now names it.
...
[Unreleased]: https://github.com/ycpss91255-docker/base/compare/v0.43.0-rc2...HEAD
[v0.43.0-rc2]: https://github.com/ycpss91255-docker/base/compare/v0.43.0-rc1...v0.43.0-rc2
