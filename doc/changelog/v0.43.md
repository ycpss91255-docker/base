...
- **the `[Unreleased]` category set is locked, and the split changelog has a
  ...
  the changelog-entry lint refuses any other heading in `[Unreleased]` and fails
  if `doc/changelog/CONVENTIONS.md` stops printing the same list. The new
  `changelog-layout` lint pins where a section, its compare link and
  `[Unreleased]` live, and re-derives the index. Released sections keep the
  headings they shipped with. Affects anyone writing a base changelog entry.
