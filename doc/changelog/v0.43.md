...
- **a release page carries the curated notes, and an extraction that finds
  ...
  replaces the release job's inline `awk`, which wrote an empty file when it
  matched nothing and shipped a body of autogenerated PR titles with green CI.
  A final release's notes are now the union of its RC sections, merged category
  by category, and `generate_release_notes` is off: the v0.42.0 body measured
  26,690 characters of which five lines were curated. It also refuses a body
  over GitHub's 125,000-character limit rather than letting the API reject it
  at tag push. Affects anyone reading a base release page.
