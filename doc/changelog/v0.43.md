...
- **toml_bridge.sh native fallback, deduped template, and test
  alignment (refs #1128)** -- ~90% of CI failures shared a single
  root cause: `toml_bridge.sh` always dispatched to `docker run` but
  the test-tools container has no docker socket. Added
  `_toml_bridge_use_native()` helper that tries the installed
  `/usr/local/bin/toml-bridge` binary first, falling back to
  `docker run` only when the native binary is absent or
  `TOML_BRIDGE_FORCE_DOCKER=1` is set. Also: removed 135 duplicated
  lines in `dist/setup.toml`, fixed acceptance-test build context
  (`dockerfile/` -> `.`), added `python3` to test-tools apk install,
  and aligned seven test files to the TOML config format (error
  messages, file assertions, mount_1 seed format, lint exclusions).
